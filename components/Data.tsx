...
export default function ProjectsAndCertificates() {
  ...
  async function fetchCertificates() {
    const { data, error } = await supabase.from("certificate").select("*").order('id', { ascending: true });
          ...
          <Col md={3} key={p.id} style={{ backgroundColor: "#555659" }}>
            ...
            >
              ...
              >
                ...
                {p.moreinfo !== 'None' ?(
                    <Button variant="light"  href={p.moreinfo}>
                         View More
                    </Button>
                ):(
                    <br/>
  ...
  return (
    <div style={{ background: "#555659", paddingBottom: "5%" }}>
      <div
        className="tag-head"
        style={{ textAlign: "center", marginBottom: "5%", color: "white" ,paddingTop:'5%'}}
        id = 'myproject'
      >
        My Projects
      </div>
      ...
      <div style={{display: 'flex', justifyContent: 'center', marginTop: '20px' }}>
        <Button variant="light" href="https://drive.google.com/drive/folders/1Ij7AkDBkay4OBKrb76BMiuxsfOJNIo0d?usp=sharing">
            View More
        </Button>
      </div>
